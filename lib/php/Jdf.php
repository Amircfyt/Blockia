...
class Jdf{
  ...
  public function convertToJalali($dateGregorian,$separator="-",$num="en")
  {
    if (empty($dateGregorian))
      return null;
    
    if(strpos($dateGregorian,"-")>0)
    {
      
      $array=explode("-",str_replace(" ","-",$dateGregorian));
    }
    ...
    return $this->tr_num($this->gregorian_to_jalali($array[0],$array[1],$array[2],$separator)." ".@$array[3],$num);
  }
  
  public function convertToGregorian($dateJalali,$separator="-",$num="en")
  {
     if (empty($dateJalali))
      return null;
    
    if(strpos($dateJalali,"-")>0)
    {
      
      $array=explode("-",str_replace(" ","-",$dateJalali));
    }
    ...
    return $this->tr_num($this->jalali_to_gregorian($array[0],$array[1],$array[2],$separator),$num);
  }
  
  public function date($format='Y-m-d H:i:s',$timestamp='',$T_sec=''){
    $T_sec=(($T_sec==='') ? 0:$T_sec);    /* <= رفع خطاي زمان سرور ، با اعداد '+' و '-' بر حسب ثانيه */
