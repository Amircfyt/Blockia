...
INNER JOIN {$DB->tablePrefix}exam_question on {$DB->tablePrefix}exam.id={$DB->tablePrefix}exam_question.exam_id AND {$DB->tablePrefix}exam_question.id=<bind>$question_id</bind>
WHERE  {$DB->tablePrefix}exam_result.block_user_id='$block_user_id' LIMIT 1",true);

if(!isset($exam->id))
{
    exit("<span style='color:red' >error: شماره برگه سوال متعلق به شما یافت نشد</span>");
}
