...
$exam_result=$DB->query("SELECT {$DB->tablePrefix}exam_result.*,{$DB->tablePrefix}exam.duration,
			...
			AND {$DB->tablePrefix}exam_result.block_user_id='$block_user_id' ",true);
if(isset($exam_result->id))
{
	...
	{
		//header("location:examResult.php?id=$exam_id&eri=$exam_result_id&ee");
		print_r($exam_result);
		exit;
	}
	else
...
{
	exit("خطا: این آزمون متعلق به شما نیست");
}
  ...
  <body class="<?=$dir?>" >
        <div class="container border fixed-top " style="max-width: 700px;" >
            <div class="row text-center bg-light text-primary align-items-center "  id="headerExam"   >
              ...
              <div class="col-3 p-0">
                <div id="remainedTime" >
                  <p class="text-danger" >پیش نمایش آزمون </p>
                </div>
              </div>
              <div class='col-12' >
